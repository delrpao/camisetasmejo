...
// -------- Juego 8: Memoria Exprés (versión más difícil) --------
...
function snakeHTML(){
    ...
    <div class="juego" style="display:block;text-align:center;">
        <h2>🐍 Mini Snake</h2>
        <canvas id="snakeCanvas" width="300" height="300" style="border:2px solid #444; background:#f0f0f0; border-radius:8px;"></canvas>
        ...
        <div id="controlesSnake" style="display:inline-block; text-align:center; margin-top:10px;">
            <div><button onclick="cambiarDireccion('up')" style="font-size:18px; padding:8px 12px;">⬆️</button></div>
            <div style="margin-top:5px;">
                <button onclick="cambiarDireccion('left')" style="font-size:18px; padding:8px 12px;">⬅️</button>
                <button onclick="cambiarDireccion('down')" onclick="cambiarDireccion('down')" style="font-size:18px; padding:8px 12px;">⬇️</button>
                <button onclick="cambiarDireccion('right')" style="font-size:18px; padding:8px 12px;">➡️</button>
            </div>
        </div>
        <div style="margin-top:10px;">
            <button onclick="volverAlMenu()" style="padding:8px 16px; font-size:16px; border-radius:6px;">⬅ Volver al menú</button>
        </div>
    </div>`;
    iniciarSnake();
}
...
function iniciarSnake(){
    ...
    function dibujar(){
        ...
        snake.forEach((s,i)=>{
            ...
            ctx.fillRect(s.x*tam, s.y*tam, tam-1, tam-1); // ligero espacio para ver las celdas
        });
    ...
    intervaloSnake = setInterval(mover, 120); // movimiento más rápido y fluido
...
// -------- Educativo: MateReto Mejorado con Récord -----------
...
// -------- Habilidad: Equilibrio Loco (Nueva Versión) --------
function equilibrioHTML() {
    main.innerHTML = `
    <div class="juego" style="display:block; text-align:center;">
        <h2>⚡ Equilibrio Loco</h2>
        <canvas id="equilibrioCanvas" style="width:90%; max-width:400px; height:auto; border:2px solid #444; border-radius:12px; background:linear-gradient(to bottom, #87CEEB, #bfe9ff);"></canvas>
...
function iniciarEquilibrioNuevo() {
    ...
    const w = canvas.width, h = canvas.height;
    let bola = { x: w / 2, y: h / 2, r: 15, vx: 3, vy: -3 };
    let plataforma = { x: w / 2 - 50, y: h - 30, w: 100, h: 12 };
    let puntos = 0;
    let enJuego = true;
...
//==================TRAGAPERRAS PROFESIONAL CON APUESTAS===========
...
function jugarTragaperras() {
    ...
    // Íconos de la máquina, más variedad para dificultad
    const icons = ['🍒','🍋','🍊','🍇','⭐','🔔','💎','🍀','🍉','🥭','🍍','🥝'];

    if(!megaSlotActivo){
        saldoSlot -= apuesta; // descontar apuesta
    }
    ...
    if(result[0]===result[1] && result[1]===result[2]){
        if(result[0]==='💎'){
            saldoSlot += apuesta*30; // Triple diamante, premio mayor
            msg = "💎💎💎 ¡Triple Diamante! +" + apuesta*30 + "$";
        } else {
            saldoSlot += apuesta*10;
            msg = "🎉 ¡JACKPOT! +" + apuesta*10 + "$";
        }
    } else if(result[0]===result[1] || result[1]===result[2]){
        if(Math.random()<0.1){ // menor probabilidad
            saldoSlot += apuesta*2;
            msg = "✨ ¡Dos iguales! +" + apuesta*2 + "$";
        }
        ...
        if(!megaSlotActivo){
            ...
            if(diamantesConsecutivos>=3){
                ...
                msg = "💎 Mega Slot desbloqueada! Una tirada extra!";
...
function megaSlot(apuestaOriginal){
    const iconsMega = ['🍒','🍋','🍊','🍇','⭐','🔔','💎','🍀','🍉','🥭','🍍','🥝'];
    const result = [iconsMega[Math.floor(Math.random()*iconsMega.length)],
    ...
    if(result[0]===result[1] && result[1]===result[2]){
        ...
        if(Math.random()<0.3) ganancia = apuestaOriginal*5;
        msg += ganancia>0?"¡Dos iguales! +" + ganancia + "$":"Sin premio 😢";
    } else if(result.includes('💎')){
        ganancia = apuestaOriginal*10;
    ...
    // Volvemos a la tragaperras normal
